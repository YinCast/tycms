...
function js_delete(id,link,flag){
	var tr=$("tr[rel='"+id+"']");
	tr.find("td:last a[rel=delete]").hide().after('<img src="a/loading.gif" width="16" height="16" align="absmiddle" rel="delete_loading" />');
	$.ajax({
		url:link,
		type:"GET",
		dataType:"json",
...
function js_toggle(id,link){
	var ob=$("#"+id),t;
	t=ob.hide().after('<img src="a/loading.gif" width="16" height="16" align="absmiddle" id="'+id+'_loading" />').parents("tr");
...
function dialog_editor(selector,hgt){
	...
	if(typeof hgt=='undefined')hgt=300;
	/*edt=$(selector).xheditor({
		upLinkUrl:'index.php?xhupload',
		upLinkExt:'zip,rar,txt,doc,docx,xls,xlsx,ppt,pptx,pdf',
		upImgUrl:'index.php?xhupload',
		upImgExt:'jpg,jpeg,gif,png',
		upFlashUrl:'index.php?xhupload',
		upFlashExt:'swf,flv',
		upMediaUrl:'index.php?xhupload',
...
function dialog_upload(id,limit,type,meta,thumb){
	...
	if(typeof thumb=='undefined'){
		thumb='index.php?uploadify/upload/'+meta;
	}else{
		thumb='index.php?uploadify/upload/'+meta+'/'+thumb;
	}
	switch(type){
		case 'doc':
	...
	$(id).uploadify({
		uploader:'j/'+swf,
		cancelImg:'j/cancel.png',
		folder:'/upload',
		expressInstall:'j/expressInstall.swf',
		script:thumb,
		scriptAccess:'always',
		multi:true,
		auto:true,
		...
		onComplete:function(event,queueID,fileObj,info,data){
			...
			if(typeof info=='string'&&info.indexOf('upload/')>0){
				if('doc'==type){
					ob.append('<li><img src="a/doc.gif" /><em><a href="'+info+'" target="_blank">下载</a><a href="index.php?uploadify/delete/'+$.base64.encodex(info)+'" rel="delete_img">删除</a></em></li>');
					ob.append('<input type="hidden" name="'+id.substr(1)+'_list[]" value="'+info+'" style="display:none;" />');
				}else if('flv'==type){
					ob.append('<li><img src="a/flv.gif" /><em><a href="flv.php?'+$.base64.encodex(info)+'" target="_blank">播放</a><a href="index.php?uploadify/delete/'+$.base64.encodex(info)+'" rel="delete_img">删除</a></em></li>');
					ob.append('<input type="hidden" name="'+id.substr(1)+'_list[]" value="'+info+'" style="display:none;" />');
				}else if(info.indexOf(',')>0){
					var ary=info.split(',');
					ob.append('<li><img src="'+ary[1]+'" /><em><a href="'+ary[0]+'" rel="popup">查看</a><a href="index.php?uploadify/delete/'+$.base64.encodex(ary[1])+'" rel="delete_img">删除</a></em></li>');
					for(var i in ary){
						ob.append('<input type="hidden" name="'+id.substr(1)+'_list[]" value="'+ary[i]+'" style="display:none;" />');
					}
				}else{
					ob.append('<li><img src="'+info+'" /><em><a href="'+info+'" rel="popup">查看</a><a href="index.php?uploadify/delete/'+$.base64.encodex(info)+'" rel="delete_img">删除</a></em></li>');
					ob.append('<input type="hidden" name="'+id.substr(1)+'_list[]" value="'+info+'" style="display:none;" />');
				}
			}else{
...
function update_dialog(){
	...
	if($("#ajax_frame").size()<1){
		$("body").append('<iframe id="ajax_frame" name="ajax_frame" style="display:none;" src="javascript:;"></iframe>');
	}
